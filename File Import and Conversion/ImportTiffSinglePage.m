function [X,map] = ImportTiffSinglePage(filename, varargin)
...
%   $Revision: 1.1.6.9 $  $Date: 2008/08/20 22:58:23 $
...
% Check if we are running 32-bit or 64-bit windows:
bittype = computer('arch');

if strcmp(bittype,'win64')
    [X, map, details] = my_rtifc_r2010(args);
else % if bittype is win32
    [X, map, details] = my_rtifc(args);
end
